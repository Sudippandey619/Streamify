...
// Jamendo-specific API functions (FREE - Creative Commons licensed music!)
export const searchJamendo = async (query: string, limit = 20) => {
  return apiClient.get(`/jamendo/search/tracks?q=${encodeURIComponent(query)}&limit=${limit}`);
};

export const getJamendoPopular = async (limit = 20) => {
  return apiClient.get(`/jamendo/popular?limit=${limit}`);
};

export const getJamendoNewReleases = async (limit = 20) => {
  return apiClient.get(`/jamendo/new-releases?limit=${limit}`);
};

export const getJamendoByGenre = async (genre: string, limit = 20) => {
  return apiClient.get(`/jamendo/genre/${genre}?limit=${limit}`);
};

export const testJamendoConnection = async () => {
  return apiClient.get('/jamendo/test');
};
