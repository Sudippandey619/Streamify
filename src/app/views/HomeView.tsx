...
import { getAudiusTrending, fetchTracks } from '@/services/api';
...
const HomeView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res: any = await getAudiusTrending(12);
        if (!mounted) return;
        // Audius endpoint may return different shapes, normalize
        const items = (res?.data || res?.tracks || res || []).map((t: any) => ({
          id: t.id || t.track_id || t.name || JSON.stringify(t),
          title: t.title || t.name || t.track || 'Unknown',
          artist: t.user?.name || t.artist || 'Unknown Artist',
          image: t.artwork?.url || t.coverUrl || t.thumbnail || '',
          duration: t.duration || t.duration_seconds || 0,
          previewUrl: t.preview_url || t.streamUrl || t.audioUrl || undefined,
